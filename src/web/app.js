...
// ---------------- game mode ----------------
/** Registry + per-family scoreboard layout. Loaded once; retried if it failed. */
...
/** Column set for a family, straight from the server's scoreboardColumns(). */
function scoreboardFor(family) {
  const sb = MODES && MODES.scoreboard;
  if (!sb) return [];
  const f = (family && Array.isArray(sb[family])) ? family : (MODES.defaultFamily || '');
  return Array.isArray(sb[f]) ? sb[f] : [];
}
...
function renderLive(s) {
  ...
  if (!MODES) loadModes();
  const scols = scoreboardFor(s.mode && s.mode.family);
  ...
  teamIds.forEach((t, i) => {
    const tm = meta(t, i);
    const tp = players.filter((p) => String(p.teamId) === t)
    ...
    const head = el('tr', {}, el('th', {}, 'Spieler'));
    for (const c of scols) head.append(el('th', { title: c.label || c.key }, c.short || c.key));
    tbl.append(head);
    for (const p of tp) {
      const cells = scols.map((c) => (c.received ? `${num(p[c.key])}/${num(p[c.received])}` : String(num(p[c.key]))));
      const tr = trow('td', [p.name == null ? '' : p.name, ...cells]);
      tr.firstChild.className = 'pl';
      if (sm5) { const mk = statsMark(p); if (mk) tr.firstChild.append(mk); }
...
function applyAdmin(c, pins) {
  ...
  $('admin-state').style.color = (!set && !pinned) ? 'var(--alert)' : '';
...
// The console therefore never hard-wires a column list: it takes the header of
// whatever file the server handed it, exactly like the live scoreboard takes its
// columns from the server's scoreboardColumns().
let totalsFamily = null;       // the family currently shown (null until pinned)
let totalsFamilies = [];       // families that actually have recorded data
const FAMILY_LABEL = { laserball: 'Laserball', sm5: 'SM5' };
const TOTALS_LABEL = {
  name: 'Spieler', matches: 'Matches', wins: 'S', losses: 'N', draws: 'U',
  score: 'Punkte', goals_per_match: '⌀ Tore', score_per_match: '⌀ Punkte',
  // laserball
  goals: 'Tore', assists: 'Vorlagen',
  steals_done: 'Steals', steals_received: 'Steals erl.',
  blocks_done: 'Blocks', blocks_received: 'Blocks erl.',
  resets_done: 'Resets', resets_received: 'Resets erl.',
  clears_done: 'Clears', clears_received: 'Clears erl.',
  passes_done: 'Pässe', passes_received: 'Pässe erh.',
  // sm5
  shots_hit: 'Treffer', shots_fired: 'Schüsse', misses: 'Daneben',
  times_hit: 'Getroffen', deactivations: 'Deakt.', times_deactivated: 'Deakt. erl.',
  shot_team: 'Eigenbeschuss', times_hit_by_team: 'Vom Team getr.',
  target_hits: 'Ziele getr.', target_destroys: 'Ziele zerst.',
  missile_locks: 'Lock-ons', missile_hits: 'Raketentreffer', missile_misses: 'Raketen daneben',
  missile_destroys: 'Raketen zerst.', times_missiled: 'Raketen erl.', missile_team: 'Rakete Team',
  nukes_activated: 'Nukes aktiv.', nukes_detonated: 'Nukes gez.', rapid_fires: 'Rapid Fire',
  ammo_resupplies: 'Ammo geg.', ammo_received: 'Ammo erh.',
  lives_resupplies: 'Leben geg.', lives_received: 'Leben erh.',
  team_ammo_resupplies: 'Ammo Team', team_lives_resupplies: 'Leben Team',
  penalties: 'Strafen', beacon_claims: 'Beacons', base_awards: 'Basen',
  achievements: 'Erfolge', rewards: 'Belohnungen',
};
const totalsLabel = (k) => TOTALS_LABEL[k] || String(k).replace(/_/g, ' ').replace(/^./, (c) => c.toUpperCase());

async function loadStats() {
  try {
    const q = totalsFamily ? `?family=${encodeURIComponent(totalsFamily)}` : '';
    const [tot, files] = await Promise.all([api('/api/stats/totals' + q), api('/api/stats/files')]);
    totalsFamilies = Array.isArray(tot.families) ? tot.families : [];
    // First visit: pin the console to a concrete family and ask again, so the
    // switcher and the table can never disagree about what is on screen.
    if (!totalsFamily && totalsFamilies.length) {
      totalsFamily = tot.family || totalsFamilies[0];
      return loadStats();
    }
    renderFamilySwitch();
...
/** Only offer what has data; with a single family there is nothing to switch. */
function renderFamilySwitch() {
  const wrap = $('totals-fam');
  if (!wrap) return;
  wrap.textContent = '';
  if (totalsFamilies.length < 2) { wrap.hidden = true; return; }
  wrap.hidden = false;
  for (const f of totalsFamilies) {
    const b = el('button', { type: 'button', className: f === totalsFamily ? 'on' : '' }, FAMILY_LABEL[f] || f);
    b.addEventListener('click', () => { if (totalsFamily === f) return; totalsFamily = f; loadStats(); });
    wrap.append(b);
  }
}
...
function renderTotals(rows) {
  ...
  if (!list.length) { t.append(el('tbody', {}, el('tr', {}, el('td', { style: 'color:var(--ink-2)' }, 'Noch keine aufgezeichneten Matches.')))); return; }
  ...
  const head = el('tr');
  keys.forEach((k) => head.append(el('th', { title: k }, totalsLabel(k))));
  ...
  for (const r of list) {
    ...
    keys.forEach((k, i) => {
      const v = r[k];
      const td = el('td', {}, v == null || v === '' ? '0' : String(v));
